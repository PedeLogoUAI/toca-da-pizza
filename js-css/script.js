    ...
    div.innerHTML = `
      <div class="produto-card">
        <div class="produto-info">
          <h3 class="produto-titulo">${prod.produto}</h3>
          <p class="produto-descricao">${prod.descricao}</p>
          <p class="produto-preco">R$ ${prod.valor.toFixed(2)}</p>
        </div>
...
function converterMarkdownParaHTML(texto) {
  // Primeiro converte todas as variações de linhas divisórias
  let html = texto.replace(/(\*\*\*|---)/g, '<br>');
  
  // Depois trata os outros elementos de formatação
  html = html.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')  // Negrito
             .replace(/\*(.*?)\*/g, '<em>$1</em>')             // Itálico
             .replace(/##/g, '</div><div style="text-align: justify;">'); // Justificado
  
  // Remove a primeira ocorrência de </div> (pois começamos com uma abertura)
  html = html.replace('</div>', '');
  
  // Adiciona um </div> final para fechar o último justify
  html += '</div>';
...
function exibirMediasAvaliacoes() {
  const tipos = ['produtos', 'atendimento', 'aplicativo'];
  
  tipos.forEach(tipo => {
    const media = avaliacoesMedias[tipo];
    const container = document.getElementById(`estrelas-${tipo}`);
    const mediaElement = document.getElementById(`media-${tipo}`);
    
    if (container && mediaElement) {
      mediaElement.textContent = `Média: ${media.toFixed(1)}`;
      
      // Limpa estilos anteriores
      container.querySelectorAll('.estrela').forEach(estrela => {
        estrela.classList.remove('ativo', 'meia-estrela');
        estrela.style.color = '';
        estrela.style.background = '';
      });

      // Calcula partes inteira e decimal
      const parteInteira = Math.floor(media);
      const parteDecimal = media - parteInteira;

      // Preenche estrelas inteiras
      for (let i = 0; i < parteInteira; i++) {
        const estrela = container.querySelector(`.estrela[data-valor="${i + 1}"]`);
        if (estrela) {
          estrela.classList.add('ativo');
          estrela.style.color = '#ffc107'; // Estrela cheia
        }
      }

      // Trata meia estrela (se houver parte decimal)
      if (parteDecimal > 0 && parteDecimal < 1) {
        const estrelaMeia = container.querySelector(`.estrela[data-valor="${parteInteira + 1}"]`);
        if (estrelaMeia) {
          estrelaMeia.classList.add('meia-estrela');
          estrelaMeia.style.background = `
            linear-gradient(90deg, #ffc107 50%, #ccc 50%)
          `;
          
      //    estrelaMeia.style.background = `
  // linear-gradient(90deg, #ffc107 ${parteDecimal * 100}%, #ccc ${parteDecimal * 100}%)
//`;
          
          estrelaMeia.style.webkitBackgroundClip = 'text';
          estrelaMeia.style.color = 'transparent';
        }
      }
    }
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Cria e posiciona a descrição do perfil
